...
import com.github.jinahya.oracle.sample.schemas.co._PersistenceUnit_ITUtils;
...
public abstract class _MappedCoEntity_PersistenceIT<ENTITY extends _MappedCoEntity<ID>, ID>
        extends __MappedEntity_PersistenceIT<ENTITY, ID> {

    protected _MappedCoEntity_PersistenceIT(final Class<ENTITY> entityClass, final Class<ID> idClass) {
            ...
            applyEntityManager(em -> ___JakartaPersistence_TestUtils.applyConnection(
                    ...
                    c -> {
                        try (var statement = c.prepareStatement("ALTER SESSION SET CURRENT_SCHEMA = ?")) {
                            statement.setString(1, getEntityTableName());
