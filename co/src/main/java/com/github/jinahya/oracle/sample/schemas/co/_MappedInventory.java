...
@MappedSuperclass
abstract class _MappedInventory<SELF extends _MappedInventory<SELF>> extends __MappedEntity<SELF, Long> {
    ...
    /**
     * The name of the table column to which the {@link _MappedInventory_#inventoryId inventoryId} attribute maps. The
     * value is {@value}.
     */
...
//                ",product=" + product +
    ...
    public final boolean equals(final Object obj) {
        if (!(obj instanceof _MappedInventory)) {
            return false;
        }
    ...
    public SELF adjustProductInventoryBy(final int delta) {
        setProductInventory(
    ...
    public SELF increaseProductInventoryBy(final int delta) {
        if (delta < 0L) {
            throw new IllegalArgumentException("negative delta: " + delta);
        }
        return (SELF) adjustProductInventoryBy(+delta);
    }

    public SELF decreaseProductInventoryBy(final int delta) {
        if (delta < 0L) {
            throw new IllegalArgumentException("negative delta: " + delta);
        }
        return (SELF) adjustProductInventoryBy(-delta);
